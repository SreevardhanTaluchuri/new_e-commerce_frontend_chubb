...
const changeQuantity = (event) => {
    const cart : cart[] = getDataFromLocalStorage("cart");
...
const render_total_cart = () => {
    ...
    let total = 0;
    (document.getElementsByClassName("cartInfoWrapper")[0] as HTMLElement).style.display="none";
    if(cart.length > 0){
        (document.getElementsByClassName("cartInfoWrapper")[0] as HTMLElement).style.display="block";
...
const getDataFromLocalStorage = (key : string) => JSON.parse(window.localStorage.getItem(key) || "[]");
const setDataInLocalStorage = (key : string , value : string) => window.localStorage.setItem(key , value);
...
const addProductsInUI = (productsData,appendTo) => {
    if(productsData.length < 1){
        ...
        productsData?.sort((a,b) => a.id-b.id)?.forEach((item , index) => {
...
const render_data_in_cart_ui = () => {
    ...
    addProductsInUI(cartData , document.getElementById("cartProducts") as HTMLDivElement);
}
